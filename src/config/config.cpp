...
/*   Updated: 2024/01/04 23:40:58 by maboulkh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
public:
    serverContext(ConfigParser &parent) : parent(parent) {
        listen = 80;
        error_page = "";
        location.insert(std::pair<string, locationContext>("/", locationContext()));
    }
    ~serverContext() {}
    void setServerName() {
        string token;
...
class ConfigParser {
public:
    ConfigParser(const std::string& filePath) : filePath(filePath), lineNumber(0), lineEnded(false), ignoreError(true) { 
    }
    ...
    void parseMainContext() {
        ...
        while (true) {
            ...
            if (token.empty())
                break;
            if (lineEnded && token != ";") {
                std::cerr << "Error: Invalid token '" << token << "' after ';' at line " << lineNumber << std::endl;
                throw std::exception();
            }
    ...
    void parseLocationContext() {
        if (scopes.back() != LOCATION || scopes.back() != SERVER) {
            std::cerr << "Error: Invalid context 'location' at line " << lineNumber << std::endl;
            throw std::exception();
        }
